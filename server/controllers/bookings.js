  ...
  try {
    ...
    if (user.id !== booking.user.id) {
      return res.sendApiError({
        title: 'Invalid User!',
        detail: 'You are not the owner of this booking!'
      });
    }
    // 29/01/2020 - 28/01/2020 - 3 days diff only delete enable
    const daysDiff = moment(booking.startAt).diff(moment(), 'days');
...
exports.createBooking = async (req, res) => {
  ...
  if (!checkIfBookingDatesAreValid(booking)) {
    return res.sendApiError({
      title: 'Invalid Booking!',
      detail: 'Choosen dates are not valid!'
    });
  }
...
function checkIfBookingIsValid(pendingBooking, rentalBookings) {
  ...
  if (rentalBookings && rentalBookings.length > 0) {
    isValid = rentalBookings.every((booking) => {
      ...
      return (
        (bookingStart < pendingStart && bookingEnd < pendingEnd) ||
